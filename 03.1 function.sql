...
DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여', '3', '남', '4', '여') || '자' 성별
FROM EMPLOYEE;

SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY 급여,
DECODE(JOB_CODE, 'J5', SALARY*1.2, 'J6', SALARY*1.15, 'J7', SALARY*1.1, SALARY*1.05) "인상된 급여"
FROM EMPLOYEE;

SELECT SUM(CASE WHEN DEPT_CODE='D5' THEN SALARY END) D5급여
FROM EMPLOYEE;

SELECT SUM(SALARY) AS D5급여
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

SELECT SUM(CASE WHEN DEPT_CODE='D5' THEN (SALARY*12 + SALARY*NVL(BONUS, 0)) END) D5총연봉
FROM EMPLOYEE;

SELECT SUM(SALARY*12) AS D5총연봉
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

SELECT TO_CHAR(SUM(SALARY), 'L999,999,999') 전사원연봉합
FROM EMPLOYEE;